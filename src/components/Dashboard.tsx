...
export default function Dashboard() {
  ...
  const handlePredict = () => {
    ...
    setTimeout(() => {
      // Generate some semi-random but plausible data
      const newScore = (Math.random() * 1.5 + 3.5).toFixed(1);
      const newTrend = Array.from({ length: 7 }, () => Math.floor(Math.random() * 40) + 50);
      
      // Factors should sum to roughly 100 or be proportional
      const f1 = Math.floor(Math.random() * 30) + 30;
      const f2 = Math.floor(Math.random() * 20) + 20;
      const f3 = 100 - f1 - f2;
      ...
      setPredictionData({
        ...
        trend: newTrend,
        factors: [f1, f2, f3],
        confidence: parseFloat((Math.random() * 5 + 94).toFixed(1))
      });
      ...
      >
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6 md:space-y-10">
          ...
          <button 
            ...
            className={cn(
              "w-full h-16 primary-gradient text-white rounded-2xl title-sm flex items-center justify-center gap-3 shadow-ambient transition-all active:scale-[0.98] cursor-pointer",
              isPredicting ? "opacity-70 cursor-wait" : "hover:opacity-95"
            )}
          >
            {isPredicting ? (
              <motion.div
            ...
            {isPredicting ? "Analyzing..." : "Predict QoE"}
          </button>
        </div>
            ...
            <div className="relative w-48 h-48 md:w-64 md:h-64 mb-8 md:mb-12">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                ...
                <motion.circle
                  ...
                  animate={{ strokeDashoffset: 282.7 - (282.7 * (predictionData.score / 5)) }}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <motion.span 
                  key={predictionData.score}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="display-lg text-on-surface mb-2"
                >
                  {predictionData.score} / 5
                </motion.span>
                <div className="px-4 py-1 bg-primary/10 text-primary rounded-full label-sm font-bold mt-2">
                  {predictionData.score >= 4.5 ? "EXCELLENT" : predictionData.score >= 4 ? "GREAT" : "GOOD"}
                </div>
              </div>
            </div>
            ...
            <div className="bg-surface-container-low p-6 md:p-8 rounded-[2rem] space-y-6 md:space-y-8">
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={predictionData.trend.map((val, i) => ({ day: trendData[i].day, value: val }))}
                  ...
                  >
                    ...
                    <Bar dataKey="value" radius={[6, 6, 0, 0]} barSize={32}>
                      {predictionData.trend.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
              ...
              {[
                {
                  ...
                  desc: predictionData.score < 4.5 ? "Reduce latency below 50ms for stable WebRTC." : "Current latency is optimal for peak performance.",
                  icon: <Activity size={18} />,
                },
                {
                  title: "Bandwidth Cap",
                  desc: predictionData.score < 4.5 ? "Increase DL speed to 10 Mbps for 1080p playback." : "Bandwidth headroom is sufficient for current load.",
                  icon: <Download size={18} />,
                },
                {
                  title: "Signal Strength",
                  desc: predictionData.score < 4.5 ? "Stabilize Jitter within 2ms range for gaming." : "Jitter stability is within the excellent range.",
                  icon: <Network size={18} />,
                },
                {
                  title: "Protocol Choice",
                  desc: predictionData.score < 4.5 ? "Switch to QUIC protocol to handle 1% packet loss." : "Current protocol stack is highly efficient.",
            ...
            <AnimatePresence>
              {!isPredicting && (
                <motion.div
                  ...
                  className="absolute top-[-15px] right-[-15px] bg-surface-container-lowest p-4 rounded-2xl shadow-ambient flex items-center gap-4 border border-outline-variant/5 z-10"
                >
